                ...
                <div class="relative z-10 w-full max-w-5xl px-6">
                    ...
                    <form id="filterForm" class="bg-white rounded-2xl shadow-lg p-6 md:p-8 grid gap-4">
                        ...
                        <div class="grid md:grid-cols-4 sm:grid-cols-2 gap-4">
                            <input type="text" id="keyword" name="keyword" placeholder="Lokasi, nama kontrakan..." class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm">
                            <select id="gender" name="gender" class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm">
                        ...
                        document.getElementById('filterForm').addEventListener('submit', function(e) {
                            ...
                            fetch(`/filter-forum?keyword=${encodeURIComponent(keyword)}&gender=${encodeURIComponent(gender)}`)
                                ...
                                .then(res => {
                                    ...
                                    container.innerHTML = '';
                                    if (data.length === 0) {
                                        container.innerHTML = '<div class="col-span-3 text-center text-gray-500">Tidak ada Forum ditemukan.</div>';
                                    } else {
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        @foreach ($groups as $group)
                            <div class="card bg-white p-5 rounded-lg shadow-md">
                                ...
                                <div class="border-t pt-4">
                                    <div class="flex justify-end gap-4">
                                        @if (auth()->check() && $checkGroup && $group->users->contains(auth()->user()->id) && $checkGroup->status == 'diterima')
                                        ...
                                        @elseif (auth()->check() && $checkGroup && $group->users->contains(auth()->user()->id) && $checkGroup->status == 'pending')
